/*
 * infinite_loop_kalman_style_MD_static.c
 *
...
#include "matrix_initialize_MD_static.h"
#include "dataMD_static_debug.h"
 ...
 */
void infinite_loop_kalman_style_MD_static(	volatile uint8_t* ADC1HalfBuffer, volatile uint8_t* ADC1FullBuffer,
											...
											uint32_t BUFFER_SIZE) // Should BUFFER_SIZE really be 32 long. The for loops below are 16 long.
{
	...
	struct DataMD_static Data;
	uint32_t maxDim = 4;
	// Remember to change all the values in the struct when changing this parameter
	// As well as the a and b vectors.
	uint32_t dim = 4;
	...
	dataMD_static_debug_observable(&Data,dim, maxDim);

	/* ------------------------------------	*/
	/* 			End of debug area 			*/
	/* ____________________________________	*/

	matrix_initialize_MD_static(&Matrix_Data, &Data, maxDim);
	...
	for(;;)
		{
			...
			{
				kalman_filter_MD_static( &Matrix_Data, (float32_t)ADC1InBuff[i]);
			...
			*ADC1HalfBuffer = 0;
			...
			for(uint16_t i=0; i<BUFFER_SIZE/2; i++)
			{
				kalman_filter_MD_static( &Matrix_Data, (float32_t)ADC1InBuff[i+BUFFER_SIZE/2] );
